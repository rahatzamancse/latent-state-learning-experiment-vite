...
const info = <const>{
  ...
  parameters: {
    ...
    /**
     * The amount of time in milliseconds to wait before listening for a response. If null, the plugin
     * will begin listening for a response immediately upon the display of the stimulus. If this value
     * is non-null, then the plugin will wait for that amount of time before beginning to listen for a response.
     */
    input_register_after: {
      type: ParameterType.INT,
      default: null,
    },
    /**
...
class HtmlKeyboardResponsePlugin implements JsPsychPlugin<Info> {
  ...
  trial(display_element: HTMLElement, trial: TrialType<Info>) {
    ...
    if (trial.choices != "NO_KEYS") {
      ...
      if (trial.input_register_after !== null && trial.input_register_after !== undefined) {
  ...
  private create_simulation_data(trial: TrialType<Info>, simulation_options: any) {
    const default_data = {
      stimulus: trial.stimulus,
      rt: this.jsPsych.randomization.sampleExGaussian(500, 50, 1 / 150, true),
      response: this.jsPsych.pluginAPI.getValidKey(trial.choices ? trial.choices : 'ALL_KEYS'),
    };
  ...
  private simulate_data_only(trial: TrialType<Info>, simulation_options: any) {
    const data = this.create_simulation_data(trial, simulation_options);

    this.jsPsych.finishTrial(data);
  }

  private simulate_visual(trial: TrialType<Info>, simulation_options: any, load_callback: () => void) {
    const data = this.create_simulation_data(trial, simulation_options);
