...
const info = <const>{
  ...
  parameters: {
    ...
    /**
     * The number of milliseconds to wait before enabling the buttons. If null, the buttons will be enabled immediately upon the display of the stimulus.
     */
    show_button_after: {
    ...
    button_html: {
      ...
      default: function (choice: string, choice_index: number) {
        return `<button class="jspsych-btn" style="margin: 10px">${choice}</button>`;
      },
    },
...
class HtmlButtonResponsePlugin implements JsPsychPlugin<Info> {
  ...
  trial(display_element: HTMLElement, trial: TrialType<Info>) {
    ...
    if (trial.show_button_after! > 0) {
      this.jsPsych.pluginAPI.setTimeout(() => {
        buttonGroupElement.style.visibility = "visible";
      }, trial.show_button_after!);
    }
    else {
      buttonGroupElement.style.visibility = "visible";
    }
    ...
    if (trial.button_layout === "grid") {
      ...
      const n_cols =
        trial.grid_columns === null || trial.grid_columns === undefined
          ? Math.ceil(trial.choices.length / trial.grid_rows!)
          : trial.grid_columns;
      const n_rows =
        trial.grid_rows === null || trial.grid_rows === undefined
    ...
    if (trial.prompt !== null && trial.prompt !== undefined) {
    ...
    // store response
    var response: { rt: null | number, button: null | number } = {
      rt: null,
      button: null,
    };
